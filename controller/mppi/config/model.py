...
class DynamicBicycleModel:
    ...
    #TODO : 조향각 , 속도를 입력으로 받도록 다시 작성.
        ...
        # dt for rate calculation (hardcoded, should match solver dt)
        dt = 0.025

        x, y, delta, v, yaw, yaw_rate, slip_angle = state
        # Control is now [delta_target, v_target] instead of [delta_rate, accel]
        delta_target, v_target = control
        
        # Compute rates from targets (P-control style)
        # Clip delta_target to valid range before computing rate
        delta_target = jnp.clip(delta_target, -0.4189, 0.4189)  # MAX_STEER
        v_target = jnp.clip(v_target, 0.0, 20.0)  # MAX_SPEED
        
        # Rate = (target - current) / dt, with rate limits applied later
        delta_v = (delta_target - delta) / dt
        a = (v_target - v) / dt
        
        # Apply rate limits
        delta_v = jnp.clip(delta_v, -3.2, 3.2)  # MAX_DSTEER
        a = jnp.clip(a, -9.0, 9.0)  # MAX_ACCEL
