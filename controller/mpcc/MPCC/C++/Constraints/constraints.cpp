...
OneDConstraint Constraints::getTireConstraintRear(const State &x) const
{
    ...
    const NormalForces f_normal = model_.getForceNormal();
...
ConstrainsMatrix Constraints::getConstraintsWithCorridorAtIndex(const ArcLengthSpline& track, const State& x, const Input& u, const CorridorData& C, int idx) const
{

    // 트랙 중심/법선 계산(기존과 동일)
    const double s = x.s;
    const Eigen::Vector2d pC = track.getPostion(s);
    const Eigen::Vector2d dC = track.getDerivative(s);
    const Eigen::Vector2d nC(-dC(1), dC(0));


    
    double r_out = C.d_left[idx];
    double r_in  = C.d_right[idx];

    if (r_out < 0.0) r_out -= param_.safety_margin;
    else r_out += param_.safety_margin;

    if (r_in < 0.0) r_in -= param_.safety_margin;
    else r_in += param_.safety_margin;

    const Eigen::Vector2d p_out = pC + r_out * nC;
    const Eigen::Vector2d p_in  = pC - r_in  * nC;

    C_i_MPC C_track = C_i_MPC::Zero();
    C_track(0,0) = nC(0); C_track(0,1) = nC(1);

    const double lower = nC.dot(p_in);
    const double upper = nC.dot(p_out);

    // 나머지 타이어/슬립각 제약 결합(기존 그대로)
    const OneDConstraint tire_r = getTireConstraintRear(x);
    const OneDConstraint alpha_f= getAlphaConstraintFront(x);

    C_MPC C_constrains_matrix; d_MPC dl_constrains_matrix, du_constrains_matrix;
    C_constrains_matrix.row(si_index.con_track) = C_track; dl_constrains_matrix(si_index.con_track) = lower; du_constrains_matrix(si_index.con_track) = upper;
    C_constrains_matrix.row(si_index.con_tire)  = tire_r.C_i; dl_constrains_matrix(si_index.con_tire) = tire_r.dl_i; du_constrains_matrix(si_index.con_tire) = tire_r.du_i;
    C_constrains_matrix.row(si_index.con_alpha) = alpha_f.C_i; dl_constrains_matrix(si_index.con_alpha)= alpha_f.dl_i; du_constrains_matrix(si_index.con_alpha)= alpha_f.du_i;
