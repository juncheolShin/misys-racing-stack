...
struct CorridorData {
    std::vector<double> d_left;   // len >= N (혹은 N+1)
    std::vector<double> d_right;  // len >= N (혹은 N+1)
    double init_s = 0.0;          // corridor가 유효한 기준 s
};
...
public:
    ...
    ConstrainsMatrix getConstraintsWithCorridorAtIndex(const ArcLengthSpline& track, const State& x, const Input& u, const CorridorData& C, int idx) const;
