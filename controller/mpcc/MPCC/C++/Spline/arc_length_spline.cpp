...
double ArcLengthSpline::getCurvature(const double s_in) const
{
    // s를 트랙 길이로 래핑(안전)
    const double s = unwrapInput(s_in);

    // 1차/2차 도함수 얻기
    const Eigen::Vector2d ds  = getDerivative(s);         // (x'(s), y'(s))
    const Eigen::Vector2d dds = getSecondDerivative(s);   // (x''(s), y''(s))

    // 곡률 공식: (x' y'' - y' x'') / ( (x'^2 + y'^2)^(3/2) )
    const double num = ds(0) * dds(1) - ds(1) * dds(0);
    const double den_sq = ds(0) * ds(0) + ds(1) * ds(1);

    // 분모 안정화
    static constexpr double kEps = 1e-12;
    if (den_sq < kEps) {
        // 아주 드물게 스플라인 미분이 0에 수렴하면 곡률을 0으로
        return 0.0;
    }
    const double den = std::pow(den_sq, 1.5);
    return num / den;  // 부호 포함 곡률 (좌회전 +, 우회전 -)
}
