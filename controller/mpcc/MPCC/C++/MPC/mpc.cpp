...
MPCReturn MPC::runMPC(State &x0)
{
    ...
    int solver_status = -1;      // 0: OK, 1: acceptable, else: not solve
    ...
    auto t_guess_start = std::chrono::high_resolution_clock::now();
    if(valid_initial_guess_)
        updateInitialGuess(x0);
    else
        generateNewInitialGuess(x0);
    auto t_guess_end = std::chrono::high_resolution_clock::now();

    n_no_solves_sqp_ = 0;
    auto t_sqp_start = std::chrono::high_resolution_clock::now();
    ...
    for(int i=0;i<n_sqp_;i++)
    {
        ...
        // std::cout << "[SQP iter " << i << "] setMPCProblem: "
        //           << std::chrono::duration<double, std::milli>(t_stage_end - t_stage_start).count() << " ms, "
        //           << "solve: "
        //           << std::chrono::duration<double, std::milli>(t_solve_end - t_solve_start).count() << " ms\n";
    }

    
    auto t_sqp_end = std::chrono::high_resolution_clock::now();
    ...
    std::chrono::duration<double> time_total = t_end - t_start;
    MPCReturn out;
    out.u0 = initial_guess_[0].uk;
    out.mpc_horizon = initial_guess_;
    out.time_total = time_total.count();
    out.solver_status = solver_status; // 마지막 SQP 반복의 상태코드
    return out;
}
...
std::vector<double> MPC::prepareInitialGuess(const State &xin) {
    State x0 = xin;
    x0.s = track_.porjectOnSpline(x0);
    x0.unwrap(track_.getLength());
    if(valid_initial_guess_) updateInitialGuess(x0);
    else generateNewInitialGuess(x0);
    std::vector<double> s_out; s_out.reserve(N+1);
    for(int i=0;i<=N;i++) s_out.push_back(initial_guess_[i].xk.s);
    return s_out;
}

MPCReturn MPC::runMPCWithCorridor(State &x0, const CorridorData &C) {
    auto t_start = std::chrono::high_resolution_clock::now();
    ...
    // setMPCProblem() 변형: 각 stage에서 corridor 기반 track 제약
    #pragma omp parallel for
    for(int i = 0; i <= N; i++) {
        stages_[i].nx = NX;
        stages_[i].nu = NU;
        stages_[i].ng = (i==0) ? 0 : NPC;
        stages_[i].ns = (i==0) ? 0 : NS;

        State xk_nz = initial_guess_[i].xk;
        xk_nz.vxNonZero(param_.vx_zero);

        stages_[i].cost_mat = normalizeCost(cost_.getCost(track_,xk_nz,i));
        stages_[i].lin_model = normalizeDynamics(model_.getLinModel(xk_nz,initial_guess_[i].uk));

        //  corridor track constraint 사용
        stages_[i].constrains_mat = normalizeCon(constraints_.getConstraintsWithCorridorAtIndex(track_, xk_nz, initial_guess_[i].uk, C, i));

        stages_[i].l_bounds_x = normalization_param_.T_x_inv*bounds_.getBoundsLX();
        stages_[i].u_bounds_x = normalization_param_.T_x_inv*bounds_.getBoundsUX();
        stages_[i].l_bounds_u = normalization_param_.T_u_inv*bounds_.getBoundsLU();
        stages_[i].u_bounds_u = normalization_param_.T_u_inv*bounds_.getBoundsUU();
        stages_[i].l_bounds_s = normalization_param_.T_s_inv*bounds_.getBoundsLS();
        stages_[i].u_bounds_s = normalization_param_.T_s_inv*bounds_.getBoundsUS();

        stages_[i].l_bounds_x(si_index.s) = normalization_param_.T_x_inv(si_index.s,si_index.s)*
            (initial_guess_[i].xk.s - param_.s_trust_region);
        stages_[i].u_bounds_x(si_index.s) = normalization_param_.T_x_inv(si_index.s,si_index.s)*
            (initial_guess_[i].xk.s + param_.s_trust_region);
    }
    ...
    for(int i=0;i<n_sqp_;i++) {
        ...
        if(solver_status <= 1) initial_guess_ = sqpSolutionUpdate(initial_guess_, optimal_solution_);
    }

    const int max_error = std::max(n_sqp_-1,1);
    if(n_no_solves_sqp_ >= max_error) n_non_solves_++;
    else n_non_solves_ = 0;
    if(n_non_solves_ >= n_reset_) valid_initial_guess_ = false;
