...
def analyse_tires(model, v_x, v_y, omega, delta):
    ...
    # 논문에서 steady state를 가정하고 데이터를 생산한다는 부분이 아래 두 줄에 해당하는 듯?? (stead state 일때만 아래처럼 횡력을 계산할 수 있엇던 것 같은데?) -> 이거는 한 번 확인해보기
...
def pacejka_error(params, *args):
    ...
    # 타이어 횡력을 reference로 least squre 최적화를 정의 (Pacejka 모델의 예측 횡력과 steady state data generation에 의해 생성된 횡력 사이의 error를 정의)
